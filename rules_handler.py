...
async def inline_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query == "rules":
        for lang, rules in RULES.items():
            ...
            reject_button = InlineKeyboardButton(text="❌ Reject Rules", callback_data=f"reject_rules:{lang}")
            results.append(
                InlineQueryResultArticle(
                    id=str(uuid.uuid4()),
                    title=f"{lang} Rules",
                    input_message_content=InputTextMessageContent(rules),
                    reply_markup=InlineKeyboardMarkup([[accept_button, reject_button]])
                )
            )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if data.startswith("accept_rules:"):
            try:
                await query.answer("✅ Accepted", show_alert=False)
            except Exception as e:
                print(f"[query.answer failed] {e}")
            try:
                await query.edit_message_text(f"✅ {user} accepted rules")
            except Exception as e:
            ...
            try:
                await query.answer(f"❌ Rejected by: {user}", show_alert=True)
            except Exception as e:
                print(f"[query.answer failed] {e}")
            try:
                await query.edit_message_text(f"❌ {user} rejected rules")
            except Exception as e:
                print(f"Edit failed: {e}")
            with open("rules_accept_log.txt", "a", encoding="utf-8") as log_file:
                log_file.write(f"{datetime.now().isoformat()} - Rejected by: {user} ({lang})\n")
