        ...
        for lang, rules in RULES.items():
            ...
            results.append(
                InlineQueryResultArticle(
                    ...
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="✅ Accept Rules", callback_data=f"accept_rules:{lang}"), InlineKeyboardButton(text="❌ Reject Rules", callback_data=f"reject_rules:{lang}")]])
                )
            )
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(button_handler, pattern="^(accept_rules|reject_rules):"))
