...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [InlineKeyboardButton("Set Numbers", callback_data="setnumbers"),
         InlineKeyboardButton("Check Now", callback_data="checknow")],
        [InlineKeyboardButton("Update Bot", callback_data="updatebot")]
    ]
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    if query.data == "setnumbers":
        await query.edit_message_text("Use /setnumbers followed by numbers to set tracking list.")
    elif query.data == "checknow":
        await check_now(update, context)
    elif query.data == "updatebot":
        try:
            subprocess.run(["git", "pull"], check=True)
            await query.edit_message_text("✅ Bot updated via git pull.")
        except subprocess.CalledProcessError as e:
            await query.edit_message_text(f"❌ Update failed: {str(e)}")
